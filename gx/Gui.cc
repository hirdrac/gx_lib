...
static void updateSize(GuiElem& def, const GuiTheme& thm)
{
  // calculate child sizes before parent
  for (GuiElem& e : def.elems) { updateSize(e, thm); }
  ...
  switch (def.type) {
    ...
    case GUI_LABEL: {
      ...
      const auto props = tf.calcProperties(label.text);
      def._w = std::max(props.width, label.minLength);
      def._h = std::max(std::max(props.height, minHeight) - 1.0f, 0.0f);
      break;
    }
    case GUI_VLABEL: {
      const auto& label = def.label();
      ...
      const TextFormat tf{
        .font = thm.font, .lineSpacing = float(thm.textSpacing)};
      const auto props = tf.calcProperties(label.text);
      def._w = std::max(std::max(props.height, minHeight) - 1.0f, 0.0f);
      def._h = std::max(props.width, label.minLength);
      break;
    }
    case GUI_HLINE:
...
static void updatePos(GuiElem& def, const GuiTheme& thm,
  ...
  switch (def.type) {
    case GUI_HFRAME: {
      ...
      for (GuiElem& e : def.elems) {
        ...
        updatePos(e, thm, left, top, right - total_w, bottom);
    ...
    case GUI_VFRAME: {
      ...
      for (GuiElem& e : def.elems) {
        ...
        updatePos(e, thm, left, top, right, bottom - total_h);
    ...
    case GUI_CHECKBOX:
      ...
      updatePos(def.elems[0], thm, left, top, right, bottom);
      break;
    case GUI_MENU: {
      GuiElem& e0 = def.elems[0];
      updatePos(e0, thm, left, top, right, bottom);
      // always position menu frame below menu button for now
      top = e0._y + e0.layoutH() + thm.border;
      GuiElem& e1 = def.elems[1];
      updatePos(e1, thm, left, top, left + e1.layoutW(), top + e1.layoutH());
      break;
    }
    case GUI_SUBMENU: {
      const float b = thm.border;
      updatePos(def.elems[0], thm, left + b, top + b, right - b, bottom - b);
      // sub-menu items
      left += def.layoutW();
      GuiElem& e1 = def.elems[1];
      updatePos(e1, thm, left, top, left + e1.layoutW(), top + e1.layoutH());
    ...
    case GUI_LISTSELECT: {
      const float b = thm.border;
      updatePos(def.elems[0], thm, left + b, top + b, right - b, bottom - b);
      updatePos(def.elems[1], thm, left - thm.popupBorder, bottom,
                right, bottom);
      break;
    }
    default:
...
bool Gui::update(Window& win, EventState& es)
{
  ...
  for (auto& p : _panels) {
    ...
    if (p->needLayout) {
      updateSize(p->root, *p->theme);
      updatePos(p->root, *p->theme, 0, 0, p->layout.w, p->layout.h);
      p->needLayout = false;
    }
  }
  ...
  if (_heldType == GUI_TITLEBAR) {
    ...
    if (lbuttonEvent) {
      ...
      if (parent) {
        ...
        const GuiTheme& thm = *(pPtr->theme);
        const float b = thm.border;
        updatePos(e0, thm, ls._x + b, ls._y + b, ls._x + ls._w - b,
                  ls._y + ls._h - b);
...
void Gui::layout(Panel& p, float x, float y, Align align)
{
  ...
  updatePos(p.root, thm, 0, 0, p.layout.w, p.layout.h);
  _needRender = true;
}
  ...
  const Style* style)
{
  ...
  switch (def.type) {
    ...
    case GUI_ENTRY: {
      ...
      const auto props = tf.calcProperties(txt);
      ...
      if (props.width <= maxWidth) {
        if (hAlign(entry.align) == Align::right) {
          tx = ex + ew - (props.width + cw + thm.entryRightMargin);
        } else if (hAlign(entry.align) != Align::left) { // HCENTER
          tx = ex + ((ew - props.width) * .5f);
        }
      }
      float cx = 0;
      if (def._id == _focusID) {
        if (props.width <= maxWidth) { _focusEntryOffset = 0; }
        const auto p2 = tf.calcProperties(substrUTF8(txt, 0, _focusCursorPos));
        cx = tx + _focusEntryOffset + p2.width;
        if (cx < leftEdge) {
          ...
          const float tEnd = tx + _focusEntryOffset + props.width;
          if (tEnd < rightEdge) {
      ...
      if (def._id == _focusID) {
        ...
        if (_focusCursorPos != _focusRangeStart) {
          ...
          const auto p2 =
            tf.calcProperties(substrUTF8(txt, 0, _focusRangeStart));
          const float cx2 = tx + p2.width;
